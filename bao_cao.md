...
Bên cạnh đó, môn học "Các nền tảng phát triển phần mềm" đặt ra yêu cầu sinh viên phải biết ứng dụng các mô hình điện toán đám mây (Cloud Computing) tiên tiến hiện nay vào việc giải quyết các bài toán thực tiễn. Do đó, việc xây dựng một hệ thống chatbot tự động, được lưu trữ, xử lý và vận hành trực tuyến trên nền tảng đám mây Microsoft Azure không chỉ giải quyết triệt để bài toán chậm trễ trong khâu tư vấn của thị trường mỹ phẩm, mà còn đáp ứng xuất sắc các mục tiêu học thuật về Cloud, API và Backend Development của môn học.
...
- Nắm vững và áp dụng thành công kiến trúc Cloud Platform bằng cách đóng gói ứng dụng qua Docker Container và lưu trữ dữ liệu tập trung tại cơ sở dữ liệu đám mây Azure Cosmos DB.
...
| 2 | Trần An Kỳ | Backend & Cloud Engineer: Thiết kế cấu trúc Database, viết các truy vấn dữ liệu. Thiết lập và triển khai (Deploy) hệ thống lên Microsoft Azure App Service. Phân tích cấu trúc API. | 100% |
...
- **Tuần 10:** Lập trình Backend (Python). Tích hợp Database. Kiểm thử luồng đặt hàng. Đăng ký tài nguyên trên Microsoft Azure và tiến hành Deploy. Phân tích API JSON.
...
|------------------------------|---------|------------------------------------------|---------|
| Azure Bot Service | Hosting & quản lý bot | Azure App Service + Docker Container | Cùng sử dụng hạ tầng Azure PaaS |
...
| QnA Maker | Hỏi-đáp từ tài liệu FAQ | Danh sách sản phẩm từ Cosmos DB | Dữ liệu sản phẩm được nạp vào AI prompt |
...
### 2.4 Nền tảng điện toán đám mây Microsoft Azure

Để đáp ứng yêu cầu cốt lõi của môn học về việc triển khai trên Cloud Platform, hệ thống được thiết kế theo mô hình PaaS (Platform as a Service) của Microsoft Azure, giúp tách biệt hoàn toàn rủi ro quản lý hạ tầng vật lý:

- **Azure App Service:** Là dịch vụ lưu trữ ứng dụng web cực kỳ mạnh mẽ hỗ trợ đa ngôn ngữ (Python, Node.js, .NET). Nhóm đã thiết lập một Linux App Service Plan để đóng gói và triển khai mã nguồn Python thông qua Docker Container. App Service giúp bot online 24/7, cung cấp sẵn chứng chỉ bảo mật SSL/TLS và cho phép cấu hình các Biến môi trường (Environment Variables) để bảo mật `TELEGRAM_TOKEN`, `COSMOS_KEY`, `GEMINI_API_KEY` một cách an toàn nhất.
- **Azure Cosmos DB (NoSQL):** Để lưu trữ dữ liệu, nhóm sử dụng cơ sở dữ liệu phi cấu trúc Azure Cosmos DB for NoSQL. Hệ thống tổ chức dữ liệu thành 3 container: `users` (thông tin khách hàng), `products` (danh mục sản phẩm) và `orders` (đơn hàng). Cấu trúc linh hoạt của NoSQL cho phép dễ dàng thêm bớt các trường thông tin của mỹ phẩm mà không làm gãy vỡ (break) hệ thống cơ sở dữ liệu đang vận hành.
...
Hình 3.2: Kiến trúc tổng thể của hệ thống triển khai trên Microsoft Azure.
...
- **Tầng Application (Xử lý cốt lõi):** Chạy trên Azure App Service, gồm hai thành phần:
...
- **Tầng Data (Dữ liệu):** Azure Cosmos DB lưu trữ dữ liệu theo thời gian thực trong 3 container: `users`, `products`, `orders`.
...
2. Telegram API đẩy dữ liệu về Azure App Service. Bot phản hồi: "Vui lòng nhập tên sản phẩm bạn muốn đặt".
...
| Khảo sát da | `/skintype` | Hiển thị Inline Keyboard với 4 nút chọn (Da dầu, Da khô, Da hỗn hợp, Da nhạy cảm). Khi chọn, cập nhật trường `skintype` của user vào Database bằng `replace_item()`. |
| Gợi ý | `/recommend` | Đọc `skintype` hiện tại từ container `users`. Truy vấn sản phẩm bằng `ARRAY_CONTAINS(c.skintype, @s)` và trả về tối đa 8 sản phẩm phù hợp. |
...
| Tìm kiếm | `/search [từ khóa]` | Bóc tách từ khóa. Truy vấn `CONTAINS(LOWER(c.name),@k) OR CONTAINS(LOWER(c.description),@k)` trên Database. Trả về kết quả nếu tìm thấy. |
...
| Lịch sử | `/myorder` | Truy vấn container `orders` với `WHERE c.user_id=@uid ORDER BY c.created_at DESC`, giới hạn 10 đơn gần nhất. |
...
Hệ thống sử dụng cơ sở dữ liệu NoSQL Azure Cosmos DB for NoSQL với cấu trúc Document (JSON). Database có tên `lumibeauty` chứa 3 container:

**1. Container: `users` (Khách hàng) — Partition Key: `/telegram_id`**
...
**2. Container: `products` (Sản phẩm) — Partition Key: `/id`**
...
**3. Container: `orders` (Đơn đặt hàng) — Partition Key: `/user_id`**
...
Một phần cốt lõi của hệ thống Chatbot chính là quá trình trao đổi dữ liệu qua lại (Request-Response Flow) giữa máy chủ Telegram và Backend Python triển khai trên Azure.

**Cơ chế hoạt động:** Khi người dùng gửi một tin nhắn trên ứng dụng Telegram, máy chủ Telegram sẽ sinh ra một Update object và bắn một POST Request mang theo chuỗi dữ liệu định dạng JSON tới Webhook URL trên Azure App Service của nhóm.

**1. Cấu trúc JSON Request (Từ Telegram gửi tới Azure Backend):**
...
**2. Cấu trúc JSON Response (Từ Azure Backend phản hồi lại Telegram):**
...
| Logic cá nhân hóa | Yếu. Kịch bản tĩnh. Cuối cùng vẫn điều hướng ra gặp tổng đài viên con người. | Chỉ hỗ trợ trả lời FAQ (câu hỏi thường gặp), không lưu đặc điểm da cá nhân. | Mạnh mẽ. Ghi nhớ hồ sơ `skintype` của từng user vào Cloud Database. Truy xuất và đề xuất sản phẩm dựa theo dữ liệu đó bằng truy vấn `ARRAY_CONTAINS`. Kết hợp AI Gemini tư vấn dựa trên danh sách sản phẩm thực tế. |
...
| Hạ tầng Cloud | Đóng (Private Cloud của doanh nghiệp). | Máy chủ độc quyền của sàn TMĐT. | Microsoft Azure App Service + Azure Cosmos DB. Chuẩn điện toán đám mây công cộng (Public Cloud), minh bạch và dễ tự scale. |
...
- **Threats (Thách thức):** Rủi ro thay đổi chính sách từ Telegram API hoặc chi phí máy chủ Azure tăng nếu lượng người dùng vượt quá ngưỡng Free-tier. Google Gemini API có thể thay đổi model hoặc chính sách miễn phí.
...
2. **Triển khai hạ tầng không máy chủ vật lý (Serverless/PaaS):** Khuyến nghị các doanh nghiệp từ bỏ việc mua Server đặt tại cửa hàng để tránh rủi ro bảo mật và hỏng hóc phần cứng. Áp dụng giải pháp Azure App Service, doanh nghiệp chỉ phải trả tiền dựa trên lượng tài nguyên (RAM/CPU) tiêu thụ thực tế (Pay-as-you-go). Trong các đợt Flash Sale, hệ thống Azure tự động cấp phát thêm tài nguyên (Auto-Scaling) để bot không bị sập.
...
- **Tối ưu chi phí Cơ sở dữ liệu:** Sử dụng Azure Cosmos DB với gói Free Tier (cung cấp miễn phí 1000 RU/s và 25GB lưu trữ). Mức tài nguyên này dư sức quản lý 65 sản phẩm trong kho hàng, hàng nghìn bản ghi người dùng và lịch sử đơn hàng mà không phát sinh thêm chi phí duy trì hàng tháng.
- **Tối ưu chi phí Máy chủ:** Bằng cách đóng gói ứng dụng qua Docker Container (base image `python:3.10-slim` chỉ ~120MB), mã nguồn trở nên độc lập và có thể chạy thử nghiệm trên gói F1 (Free) của Azure App Service.
...
### 5.1 Thiết lập môi trường và Quy trình triển khai trên Azure

Hệ thống phần mềm được nhóm tuân thủ theo các bước quản lý vòng đời ứng dụng (ALM) cơ bản, từ viết code tại Local đến khi đưa lên Cloud (Azure).

**1. Môi trường phát triển cục bộ (Local Development):**

- **IDE:** Visual Studio Code.
- **Ngôn ngữ:** Python 3.10. Các thư viện phụ thuộc được đóng gói chuẩn chỉnh trong file `requirements.txt` bao gồm: `python-telegram-bot>=20.0`, `python-dotenv`, `azure-cosmos`, `google-generativeai`, `flask`, `requests`.
- **Bảo mật:** Biến môi trường cực kỳ nhạy cảm như `TELEGRAM_TOKEN`, `COSMOS_ENDPOINT`, `COSMOS_KEY`, `GEMINI_API_KEY` được lưu trữ an toàn trong file `.env` và được cho vào `.gitignore` để tránh bị rò rỉ (leak) khi đưa mã nguồn lên Internet. File `.env.example` được cung cấp như template cho người mới tham gia dự án.
- **Database local:** Sử dụng Azure Cosmos DB Emulator chạy tại `https://localhost:8081` để phát triển và kiểm thử mà không cần kết nối Cloud thật.
...
│   ├── __init__.py
│   ├── handlers.py         # Xử lý tất cả lệnh và callback (547 dòng)
│   ├── keyboards.py        # Định nghĩa Inline Keyboard
│   ├── messages.py         # Các thông điệp mẫu
│   └── ai.py               # Kết nối Google Gemini AI
├── database/               # Module kết nối Azure Cosmos DB
│   ├── __init__.py
│   ├── cosmos.py           # Khởi tạo client, database, container
│   ├── users.py            # CRUD container users
│   ├── products.py         # Query container products
│   └── orders.py           # CRUD container orders
├── data/
│   ├── __init__.py
...
**3. Quy trình đóng gói và đưa lên Microsoft Azure:**
...
- **Bảo mật biến môi trường:** File `.env` bị chặn bởi `.gitignore` và `.dockerignore`. Trên môi trường Cloud, các biến này được cấu hình an toàn trực tiếp vào phần Environment Variables của Azure App Service.
- **Triển khai (Deployment):** Docker Image sau khi build sẽ được đẩy lên Azure. App Service tự động kéo Image và khởi chạy container với lệnh `python app.py`. Biến `ENVIRONMENT=production` kích hoạt chế độ Webhook, bot lắng nghe trên port 8080 với `WEBHOOK_URL` từ Azure.
...
### 5.3 Phân tích hiệu suất hệ thống trên Azure (Concurrency, Scaling, Latency)
...
**2. Khả năng mở rộng tự động (Scaling Strategy):** Azure App Service hỗ trợ Auto-Scaling. Nếu CPU vượt ngưỡng 70% trong 5 phút, Azure tự động sinh thêm Instances. Khi lưu lượng giảm, số lượng máy ảo được thu hẹp (Scale-in) về 1 Instance mặc định.

**3. Tối ưu hóa độ trễ mạng (Latency):** Nhóm lựa chọn khởi tạo Azure App Service và Cosmos DB tại khu vực Southeast Asia (Singapore), gần với khu vực người dùng tại Việt Nam. Thời gian đọc từ Cosmos DB < 10ms, tổng thời gian phản hồi trung bình < 200ms.

### 5.4 Kịch bản kiểm thử hệ thống (Test Cases)

| Mã TC | Chức năng | Hành động của người dùng | Kết quả mong đợi (Expected) | Trạng thái |
|-------|-----------|--------------------------|------------------------------|------------|
| TC_01 | Start Bot | User lần đầu nhấn Start hoặc gõ `/start`. | Bot gửi lời chào có emoji, hiển thị danh sách 8 lệnh hỗ trợ. User được lưu vào container `users` với `full_name`. | PASS |
...
| TC_03 | Gợi ý SP | Gõ `/recommend` sau khi đã chọn loại da. | Bot truy vấn `ARRAY_CONTAINS` và trả về tối đa 8 sản phẩm phù hợp với loại da đã lưu, kèm giá và mô tả. | PASS |
...
| TC_08 | Order - Thành công | Gõ `/order`, nhập tên SP hợp lệ, nhập số "2", nhập họ tên, nhập địa chỉ "Quận 1, HCM", bấm "✅ Xác nhận". | Bot báo "✅ Đặt hàng thành công!" kèm mã đơn. Dữ liệu xuất hiện trên Cosmos DB với `status = "Đang xử lý"`, có `customer_name` và `created_at`. | PASS |
...
Hình 5.5.4: Dữ liệu đơn hàng trên Azure Cosmos DB Data Explorer hiển thị các trường `user_id`, `customer_name`, `product_name`, `quantity`, `address`, `status = "Đang xử lý"` và `created_at` được đồng bộ Real-time.
...
- **Về mặt tư duy hệ thống & Cloud:** Hiện thực hóa thành công các khái niệm về Cloud Computing. Sử dụng Azure Cosmos DB for NoSQL với 3 container (`users`, `products`, `orders`) và các Partition Key được thiết kế hợp lý (`/telegram_id`, `/id`, `/user_id`). Đóng gói ứng dụng bằng Docker Container và hỗ trợ hai chế độ vận hành (Polling cho local, Webhook cho production).
